...
import math
from PyQt5.QtWidgets import (QMainWindow, QApplication, QFontDialog, QStyleFactory, QAction, QMessageBox, QFileDialog,QInputDialog
                             ,QLineEdit)
...
from face_function.loadOpenFace import prepareOpenFace,face_feature
from face_function.real_recognition import real_time_recognition
import torch
import time
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        #face recognition
        self.start_cam_face_reBtn.clicked.connect(lambda: self.start_real_recognition())
        self.stop_cam_face_reBtn.clicked.connect(lambda: self.stop_real_recognition())
        self.face_recBtn.clicked.connect(lambda: self.face_recognition())
        self.face_detcBtn.clicked.connect(lambda: self.face_detection())
        self.Data_Btn.clicked.connect(lambda: self.data_adding())
        ...
        self.face_model = prepareOpenFace()
    ...
    #face recognition
    def start_real_recognition(self):
        face_names,face_features = torch.load('./face_re_data/face_features.pt')
        print(1)
        face_names = face_names
        face_features_saving = face_features
        face_features = face_features.cuda()

        length = face_features.size(0)
        self.face_model = prepareOpenFace().eval()
        self.key = 1
        vs = VideoStream(usePiCamera=-1 > 0).start()
        flag = 6
        name = None
        while True:
            if self.key == 4 and flag>=6:
                flag = 0
                name, ok = QInputDialog.getText(self, "Please input your name", "This is input hint\n\n Please input text:", QLineEdit.Normal, "This is default")
                self.echo(name)
                self.key =3
            frame = vs.read()
            frame = cv2.flip(frame,1)
            if self.key == 2 or self.key == 3:
                frame,feature = real_time_recognition(frame,self.key,self.face_model,face_features,face_names)
                # print(feature.size())
                # cv2.imshow('aa',output)

                if self.key == 3 and flag<6:
                    time.sleep(0.35)
                    flag = flag + 1
                    # print(flag)
                    ...
                    print(face_names)
                    face_features_saving = torch.cat((face_features_saving,feature),0)
                    if flag == 6:
                        file = open('./face_re_data/face_features.pt','wb')
                        torch.save((face_names,face_features_saving),file)
                        file.close()
                        self.key = 1
                        face_names,face_features = torch.load('./face_re_data/face_features.pt')
                        face_features = face_features.cuda()
            ...
            if self.key == 0:
                break
            self.displayImage2(frame, 7)
            QApplication.processEvents()
        self.key=1
        vs.stop()

    def stop_real_recognition(self):
        self.key=0
    ...
    def displayImage2(self, img, myWindow=1 ):
        ...
        if myWindow == 7:
            self.Face_re_Label.setPixmap(QPixmap.fromImage(outImage))
            self.Face_re_Label.setScaledContents(True)
...
window = MainWindow()
window.setWindowTitle('The Face Application')   # To name the window
...
window.setStyleSheet("""
                ...
                """)
window.setWindowIcon(QIcon('finallogo.png'))
window.show()
sys.exit(app.exec_())
